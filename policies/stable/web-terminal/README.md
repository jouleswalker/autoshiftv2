...
## Overview
Installs the web-terminal operator using AutoShift's ACM **PolicyGenerator** pattern. The
...
web-terminal/
  ...
  placement.yaml                # Placement predicate (autoshift.io/web-terminal) + tolerations
  manifests/
    operator-install/
...
`autoshift.io/web-terminal: 'true'`.
...
hubClusterSets:
  hub:
    labels:
      ...
      # web-terminal-version: 'web-terminal.v1.x.x'  # Optional: pin to a specific CSV
```
...
- **Pin a version** with the `web-terminal-version` label — a pinned version sets a manual
...
2. For a **static** CR, drop a bare manifest into `manifests/`:
   ```yaml
   # manifests/web-terminal-config.yaml
...
4. If the config must be its **own** Policy (separate remediation, a dependency, or a different
   ...
   `policy-web-terminal-operator-install`) and point it at the new manifest with its own
   `placement.placementPath`. `generate-policy.sh --dir policies/stable/web-terminal` scaffolds this.

Document any new `autoshift.io/web-terminal-*` label in `autoshift/values/clustersets/_example.yaml`
...
- **Policy not applied**: `oc get managedcluster <cluster> --show-labels`; check the Placement
  (`oc get placement -n policies-autoshift`) and `oc describe policy policy-web-terminal-operator-install`.
...
## Resources
- [Operator Documentation](https://operatorhub.io/operator/web-terminal)
